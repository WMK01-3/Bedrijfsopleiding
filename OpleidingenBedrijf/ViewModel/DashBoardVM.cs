...
namespace BedrijfsOpleiding.ViewModel
{
    public class DashBoardVM : BaseViewModel
    {
        ...
        private double[] _getLongLat(string street, string city, string zip)
        {
            // to Read the Stream
                ...
                while (xmlReader.Read())
                {
                    ...
                    switch (xmlReader.Name)
                    {
                        case "lat":
                            ...
                            {
                                xmlReader.Read();
                                _longlat[1] = Convert.ToDouble(xmlReader.Value);
                        ...
                        case "lng":
                            ...
                            {
                                xmlReader.Read();
                                _longlat[0] = Convert.ToDouble(xmlReader.Value);
        ...
        public void LoadMarkers(WebBrowser wbMaps)
        {
            ...
            using (var context = new CustomDbContext())
            {
                ...
                foreach (var location_id in allLocationIdFromCourses)
                {
                    ...
                    double Long = _getLongLat(location.Street, location.City, location.Zipcode)[0];
                    double Lat = _getLongLat(location.Street, location.City, location.Zipcode)[1];
